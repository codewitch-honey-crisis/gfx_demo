...
namespace gfx {
    ...
    private:
        ...
        static JRESULT mcu_output(
            ...
            for (iy = 0; iy < my; iy++)
            {
                ...
                for (ix = 0; ix < mx; ix++)
                {
                    ...
                    *ycbcr24++ = /* R */ BYTECLIP(yy + ((int)(1.402 * CVACC) * cr) / CVACC);
                    *ycbcr24++ = /* G */ BYTECLIP(yy - ((int)(0.344 * CVACC) * cb + (int)(0.714 * CVACC) * cr) / CVACC);
        ...
        static uint16_t LDB_WORD(const uint8_t* ptr) { return (uint16_t)(((uint16_t) * ((uint8_t *)(ptr)) << 8) | (uint16_t) * (uint8_t *)((ptr) + 1)); }
