	...
	)
	{
		...
		for (iy = 0; iy < my; iy++)
		{
			...
			for (ix = 0; ix < mx; ix++)
			{
				...
				*ycbcr24++ = /* Y */ yy; //BYTECLIP(yy + ((int)(1.402 * CVACC) * cr) / CVACC);
				*ycbcr24++ = /* Cb */ cb; //BYTECLIP(yy - ((int)(0.344 * CVACC) * cb + (int)(0.714 * CVACC) * cr) / CVACC);
				*ycbcr24++ = /* Cr */ cr; // BYTECLIP(yy + ((int)(1.772 * CVACC) * cb) / CVACC);
			}
		}
